...
def model_train(model, train_name):
    ...
    data_path_train_tumor = osp.join(ROOT_PATH, 'train/%s' % train_name)
    ...
    data_path_train_normal_trainset = osp.join(ROOT_PATH_NORMAL, 'train/normal/trainset_s6')
    ...
    if num_normal_train_trainset < 79000:
        print('train normal %s tiles %d not complete!' % (train_name, num_normal_train_trainset))
        return ERROR_NAME
    else:
        print('train normal %s tiles %d' % (train_name, num_normal_train_trainset))

    # get validation dataset
    data_path_val_tumor = osp.join(ROOT_PATH, 'validation/%s' % train_name)
    ...
    # get the training slides normal dataset
    data_path_val_normal_trainset = osp.join(ROOT_PATH_NORMAL, 'validation/normal/trainset_s6')
    ...
    if num_normal_val_trainset < 18000:
        print('validation normal %s tiles %d not complete!' % (train_name, num_normal_val_trainset))
        return ERROR_NAME
    else:
        print('validation normal %s tiles %d' % (train_name, num_normal_val_trainset))
    ...
    MODELFILE = 'vgg19_models/TNsep_%s.h5' % train_name

    # set steps per epoch and num epoches
    N_epochs = 20    # train size 160K with bs32 translates into 5000 steps for a full epoch
    steps_per_epoch = training_generator.N//params['batch_size']      # some are < 160k; this ensures it won't run out of data 
...
def model_test(model, MODELFILE):
    # read in data
    tiles_tumor_test = glob.glob(osp.join(ROOT_PATH_NORMAL, 'test/tumor/*.png'))
    data_tumor_test = [[x, 1] for x in tiles_tumor_test]
    num_tumor_test = len(data_tumor_test)
    print('test tumor tiles %d' % num_tumor_test)

    tiles_normal_test = glob.glob(osp.join(ROOT_PATH_NORMAL, 'test/normal/*.png'))
...
if __name__ == '__main__':
    ...
    data_path = osp.join(ROOT_PATH, 'train/%s' % train_name)
