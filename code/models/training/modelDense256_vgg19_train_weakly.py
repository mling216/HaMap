...
def model_train(model, train_name):
    ...
    seed = train_name.split('_s')[-1].split('_')[0]
    ...
    data_path_train_normal_trainset = osp.join(ROOT_PATH, 'train/normal/trainset_s6')
    ...
    #####
    random.seed(seed)
    data_normal_train_trainset = random.sample(data_normal_train_trainset, num_tumor_train)
    #####

    num_normal_train_trainset = len(data_normal_train_trainset)
    if num_normal_train_trainset < 19000:
    ...
    params = {'dim': (224,224,3),
            'batch_size': 32,
            'shuffle': False}
    val_generator = DataGenerator(train_data, **params)
    ...
    basename = 'vgg19_models/TNsep_%s' % train_name
    MODELFILE = basename + '_epoch_{epoch:02d}.h5'

    # set steps per epoch and num epoches
    ### train only 5 epochs and use the best model
    N_epochs = 5    # train size 160K with bs32 translates into 1250 steps for a full epoch
    ...
    # save history
    with open(basename + '_history.pkl', 'wb') as file_pi:
...
if __name__ == '__main__':
    ...
    if not osp.exists(data_path):
        ...
        basename = 'vgg19_models/TNsep_%s' % train_name
        for i in range(4,6):
            MODELFILE = basename + '_epoch_%02d.h5' % i
            model_test(model, MODELFILE)
