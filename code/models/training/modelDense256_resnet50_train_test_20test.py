...
def model_train(model, train_name):
    ...
    data_path_train_normal_trainset = osp.join(ROOT_PATH, 'train/normal/trainset_s%s' % normal_seed[seed])
    ...
    # get the training slides normal dataset
    data_path_val_normal_trainset = osp.join(ROOT_PATH, 'validation/normal/trainset_s%s' % normal_seed[seed])
    ...
    MODELFILE = 'resnet50_models/TNsep_%s.h5' % train_name
...
def model_test(model, MODELFILE):
    ...
    ROOT_PATH_20test = '/fs/scratch/PAS1575/Pathology/CAMELYON16/eyeTrack224StainNorm'

    # read in data
    tiles_tumor_test = glob.glob(osp.join(ROOT_PATH_20test, 'test/tumor/*.png'))
    data_tumor_test = [[x, 1] for x in tiles_tumor_test]
    num_tumor_test = len(data_tumor_test)
    print('test tumor tiles %d' % num_tumor_test)

    tiles_normal_test_tumor = glob.glob(osp.join(ROOT_PATH_20test, 'test/normal/test_tumor/*.png'))
    ...
    tiles_normal_test = glob.glob(osp.join(ROOT_PATH_20test, 'test/normal/test_normal/*.png'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', metavar='--subfolder', type=str, action='store', 
                        ...
                        help='Train dataset folder name, e.g., 20test_0.3_s1')
