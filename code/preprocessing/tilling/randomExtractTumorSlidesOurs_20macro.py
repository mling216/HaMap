...
###################################################################
#
# extract from the 20 macro slides, NOT using merged masks, but random participant's mask
#
###################################################################
...
def cal_extract_allocation(seed, weight_threshold):
    ...
    ### get masks
    df_pid = pd.read_csv('experimentData/20TestCorrectParticipants.csv')
    masks = []
    for imageID in MACRO_SLIDES:
        df_select = df_pid[df_pid['imageID']==imageID]
        imageName = df_select['imageName'].values[0]
        pid = df_select['pID'].sample(random_state=seed).values[0]
        maskName = '_'.join([imageID,pid,weight_threshold])+'.npy'
        if maskName in ['C58_P9_0.7.npy']:  # 0 tile
            pid = df_select['pID'].sample(random_state=seed+1).values[0]
            maskName = '_'.join([imageID,pid,weight_threshold])+'.npy'
        masks.append([maskName, imageName])
    ...
    for selection in slides_to_test:
        ...
        slide_file = osp.join(SLIDE_PATH, selection[1])
        ...
        # print('number of tumor tiles at threshold %s is %d' % (participant, np.sum(mask)))
    ...
    print('train tumor patches', n_train_tumor_patches)
    print('val tumor patches', n_val_tumor_patches)
    ...
    print(total_to_sample_train_tumor, total_to_sample_val_tumor)
...
def random_extract(slide_info, extract_allocation, output_path_train_tumor, 
    ...
    for allocation in extract_allocation:
        ...
        print(slide_name, num_tumor_already, num_to_extract_tumor)
...
MACRO_SLIDES = ['C1','C5','C10','C18','C19','C21','C24','C32','C33','C35','C39',
                'C41','C44','C46','C47','C49','C51','C55','C56','C58']

# ['4','10','11']   # the best, worst, and middle performer, order unknown
PARTICIPANT_SET = ['1','2','3','4','5','7','8','9','10','11']  # the rest
...
def main(seed, weight_threshold):
    ...
    random_extract(slide_info, extract_allocation, output_path_train_tumor, output_path_val_tumor)
...
if __name__ == '__main__':
    ...
    parser = ap.ArgumentParser(description='Get dataset seed and weight thresold')
    ...
    #### use seeds among [1, 22, 42] to ensure all data have a tile  
