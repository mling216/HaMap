...
def main(seed, weight_threshold):
    # create output path
    output_path_train_tumor = '/fs/scratch/PAS1575/Pathology/CAMELYON16/individualMask/train/normal/%s_s%s/' % (weight_threshold, seed)
    os.makedirs(output_path_train_tumor, exist_ok=True)
    output_path_train_normal = '/fs/scratch/PAS1575/Pathology/CAMELYON16/individualMask/train/notused/%s_s%s/' % (weight_threshold, seed)
    # os.makedirs(output_path_train_normal, exist_ok=True)

    output_path_val_tumor = '/fs/scratch/PAS1575/Pathology/CAMELYON16/individualMask/validation/normal/%s_s%s/' % (weight_threshold, seed)
    os.makedirs(output_path_val_tumor, exist_ok=True)
    output_path_val_normal = '/fs/scratch/PAS1575/Pathology/CAMELYON16/individualMask/validation/notused/%s_s%s/' % (weight_threshold, seed)
