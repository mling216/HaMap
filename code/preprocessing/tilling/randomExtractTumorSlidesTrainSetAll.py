...
def cal_extract_allocation(seed):
    ...
    for selection in slides_to_test:
        ...
        truth_slide_path = BASE_TRUTH_DIR + selection[0].replace('.tif', '_mask.tif')
    ...
    for i, slide_file in enumerate(sorted(list_slides)):
        ...
        if i == len(list_slides)-1:
            num_to_extract_tumor = round(NUM_SAMPLES*(1-VAL_PROPORTION)) - total_to_sample_train_tumor
            if len(df_samples_tumor) == 0:    # avoid last one > available
                num_to_extract_tumor = 0
            num_to_extract_normal = round(NUM_SAMPLES*((1-VAL_PROPORTION)/2)) - total_to_sample_train_normal
    ...
    for i, slide_file in enumerate(sorted(list_slides)):
        ...
        # calculate the num tiles to sample for current slide
        if i == len(list_slides)-1:
            num_to_extract_tumor = round(NUM_SAMPLES*VAL_PROPORTION) - total_to_sample_val_tumor
            if len(df_samples_tumor) == 0:    # avoid last one > available
                num_to_extract_tumor = 0
            num_to_extract_normal = round(NUM_SAMPLES*VAL_PROPORTION/2) - total_to_sample_val_normal
            if len(df_samples_normal) == 0:     # when 0 tiles, no extraction
                num_to_extract_normal = 0
        else:
